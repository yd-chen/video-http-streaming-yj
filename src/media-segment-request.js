...
function decrypt (data) {
  const buf = data.replace(/\n/g, "").split("");
  for (let i = 0; i < Math.floor(buf.length / 3); i++) {
    const c = buf[i * 3];
    buf[i * 3] = buf[i * 3 + 2];
    buf[i * 3 + 2] = c;
  }
  return decodeBase64(buf.join(""));
}
...
const handleKeyResponse = (segment, finishProcessingFn) => (error, request) => {
  var response = decrypt(request.response);
  response = new Uint8Array(response.match(/[\da-f]{2}/gi).map(function (h) {
...
export const mediaSegmentRequest = (xhr,
  ...
  if (segment.key && !segment.key.bytes) {
    const keyRequestOptions = videojs.mergeOptions(xhrOptions, {
      ...
      headers: {
        'Seg': Math.floor(Math.random() * 100000000)
      }
    });
